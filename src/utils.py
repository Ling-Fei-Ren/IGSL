...
###
def load_data(data, prefix='data/'):#导入数据
    """
    ...
    if data == 'yelp':
        ...
        file.close()
        with open(prefix + 'yelp_relation_1.pickle', 'rb') as file:
            relation1 = pickle.load(file)
        file.close()
        with open(prefix + 'yelp_relation_2.pickle', 'rb') as file:
            relation2 = pickle.load(file)
        file.close()
        with open(prefix + 'yelp_relation_3.pickle', 'rb') as file:
            relation3 = pickle.load(file)
        file.close()
        relations = [homo, relation1, relation2, relation3]
        ...
        with open(prefix + 'amz_relation_1.pickle', 'rb') as file:
            relation1 = pickle.load(file)
        file.close()

        with open(prefix + 'amz_relation_2.pickle', 'rb') as file:
            relation2 = pickle.load(file)
        file.close()

        with open(prefix + 'amz_relation_3.pickle', 'rb') as file:
            relation3 = pickle.load(file)
        file.close()
        ...
        # load the preprocessed adj_lists
        with open(prefix + 'mimic_homo_adjlists.pickle', 'rb') as file:
            homo = pickle.load(file)
        file.close()

        with open(prefix + 'mim_relation_1.pickle', 'rb') as file:
            relation1 = pickle.load(file)
        file.close()

        with open(prefix + 'mim_relation_2.pickle', 'rb') as file:
            relation2 = pickle.load(file)
        file.close()

        with open(prefix + 'mim_relation_3.pickle', 'rb') as file:
            relation3 = pickle.load(file)
        file.close()
        with open(prefix + 'mim_relation_4.pickle', 'rb') as file:
            relation4 = pickle.load(file)
        file.close()
        relations=[homo,relation1,relation2,relation3,relation4]



    return relations, feat_data, labels
...
def pick_step(idx_train, y_train, labels, adj_list, size):
    ...
    # return random.sample(idx_train,k=size)
...
def test_pcgnn(test_cases, labels, test_flag, model, batch_size, epoch, thres=0.5):
    """
...
def train_pcgnn(test_cases, labels, test_flag, model, batch_size, epoch, thres=0.5):
    """
    Test the performance of PC-GNN and its variants
    :param test_cases: a list of testing node
    :param labels: a list of testing node labels
    ...
    for iteration in range(test_batch_num):
        ...
        gnn_pred_list.extend(gnn_pred.tolist())
    error=np.where(np.array(gnn_pred_list) != labels)
    index=np.array(test_cases)[np.array(error)]
