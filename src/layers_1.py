...
class InterAgg_1(nn.Module):
    ...
    def __init__(self, config,features, feature_dim, embed_dim,
        ...
        """
        super(InterAgg_1, self).__init__()
        ...
        self.intra_agg4 = self.intra_agg4.to(self.device)
    ...
    def forward(self, nodes, labels, train_flag=True):
        ...
        # 三个子图GNN
        r1_feats,loss1 = self.intra_agg1.forward(self.features_1,self.adj_lists[0], labels, nodes,  1, train_flag)
        r2_feats,loss2 = self.intra_agg2.forward(self.features_1,self.adj_lists[1], labels, nodes,  2, train_flag)
        r3_feats,loss3 = self.intra_agg3.forward(self.features_1,self.adj_lists[2], labels, nodes,  3, train_flag)
        r4_feats,loss4 = self.intra_agg4.forward(self.features_1, self.adj_lists[3], labels, nodes, 4, train_flag)
...
#
    ...
    def forward(self, features, intra_agg,  labels, nodes, train_flag, batch_norm=True):
        ...
        return node_vec,loss

#子图嵌入
class IntraAgg_1(nn.Module):
    ...
    def __init__(self, config,features, feat_dim, embed_dim):
        ...
        :param cuda: whether to use GPU
        """
        super(IntraAgg_1, self).__init__()
        self.config=config
    ...
    def forward(self, features, intra_agg, labels, nodes, number, train_flag):
        """
